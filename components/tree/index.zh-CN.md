...
| 参数 | 说明 | 类型 | 默认值 | 版本 |
| --- | --- | --- | --- | --- |
| autoExpandParent | 是否自动展开父节点 | boolean | true | 3.0.0 |
| blockNode | 是否节点占据一行 | boolean | false | 3.15.0 |
| checkable | 节点前添加 Checkbox 复选框 | boolean | false | 3.0.0 |
| checkedKeys | （受控）选中复选框的树节点（注意：父子节点有关联，如果传入父节点 key，则子节点自动选中；相应当子节点 key 都传入，父节点也自动选中。当设置`checkable`和`checkStrictly`，它是一个有`checked`和`halfChecked`属性的对象，并且父子节点的选中与否不再关联 | string\[] \| {checked: string\[], halfChecked: string\[]} | \[] | 3.0.0 |
| checkStrictly | checkable 状态下节点选择完全受控（父子节点选中状态不再关联） | boolean | false | 3.0.0 |
| defaultCheckedKeys | 默认选中复选框的树节点 | string\[] | \[] | 3.0.0 |
| defaultExpandAll | 默认展开所有树节点 | boolean | false | 3.0.0 |
| defaultExpandedKeys | 默认展开指定的树节点 | string\[] | \[] | 3.0.0 |
| defaultExpandParent | 默认展开父节点 | bool | true | 3.4.0 |
| defaultSelectedKeys | 默认选中的树节点 | string\[] | \[] | 3.0.0 |
| disabled | 将树禁用 | bool | false | 3.4.0 |
| draggable | 设置节点可拖拽（IE>8） | boolean | false | 3.0.0 |
| expandedKeys | （受控）展开指定的树节点 | string\[] | \[] | 3.0.0 |
| filterTreeNode | 按需筛选树节点（高亮），返回 true | function(node) | - | 3.0.0 |
| loadData | 异步加载数据 | function(node) | - | 3.0.0 |
| loadedKeys | （受控）已经加载的节点，需要配合 `loadData` 使用 | string\[] | \[] | 3.7.0 |
| multiple | 支持点选多个节点（节点本身） | boolean | false | 3.0.0 |
| selectedKeys | （受控）设置选中的树节点 | string\[] | - | 3.0.0 |
| showIcon | 是否展示 TreeNode title 前的图标，没有默认样式，如设置为 true，需要自行定义图标相关样式 | boolean | false | 3.0.0 |
| switcherIcon | 自定义树节点的展开/折叠图标 | React.ReactElement | - | 3.12.0 |
| showLine | 是否展示连接线 | boolean | false | 3.0.0 |
| onCheck | 点击复选框触发 | function(checkedKeys, e:{checked: bool, checkedNodes, node, event}) | - | 3.0.0 |
| onDragEnd | dragend 触发时调用 | function({event, node}) | - | 3.0.0 |
| onDragEnter | dragenter 触发时调用 | function({event, node, expandedKeys}) | - | 3.0.0 |
| onDragLeave | dragleave 触发时调用 | function({event, node}) | - | 3.0.0 |
| onDragOver | dragover 触发时调用 | function({event, node}) | - | 3.0.0 |
| onDragStart | 开始拖拽时调用 | function({event, node}) | - | 3.0.0 |
