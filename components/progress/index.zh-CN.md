...
| 属性 | 说明 | 类型 | 默认值 | 版本 |
| --- | --- | --- | --- | --- |
| type | 类型，可选 `line` `circle` `dashboard` | string | `line` | 3.0.0 |
| format | 内容的模板函数 | function(percent, successPercent) | `percent => percent + '%'` | 3.0.0 |
| percent | 百分比 | number | 0 | 3.0.0 |
| showInfo | 是否显示进度数值或状态图标 | boolean | true | 3.0.0 |
| status | 状态，可选：`success` `exception` `normal` `active`(仅限 line) | string | - | 3.0.0 |
